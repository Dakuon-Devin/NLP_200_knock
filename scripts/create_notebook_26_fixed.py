...
show_emphasis_markup_examples()'''))
    ...
    # Add code cell for regex explanation
    cells.append(nbf.v4.new_code_cell('''# 正規表現のパターンを詳しく説明

def explain_regex_patterns():
    """強調マークアップを除去する正規表現のパターンを詳しく説明する関数"""
    patterns = [
        (r"\'\'\'\'\'(.+?)\'\'\'\'\'", "強い強調かつ斜体（5つのアポストロフィ）"),
        (r"\'\'\'(.+?)\'\'\'", "強い強調（3つのアポストロフィ）"),
        (r"\'\'(.+?)\'\'", "弱い強調（2つのアポストロフィ）")
    ]
    
    print("強調マークアップを除去する正規表現のパターン:")
    for pattern, description in patterns:
        print(f"\\n【パターン】{pattern}")
        print(f"【説明】{description}")
        print("【パターンの解釈】")
        if pattern == r"\'\'\'\'\'(.+?)\'\'\'\'\'":
            print("- \\\'\\\'\\\'\\\'\\\'：5つのアポストロフィにマッチ（エスケープが必要）")
            print("- (.+?)：1つ以上の任意の文字にマッチし、非貪欲（non-greedy）でグループ化")
            print("- \\\'\\\'\\\'\\\'\\\'：5つのアポストロフィにマッチ（エスケープが必要）")
        elif pattern == r"\'\'\'(.+?)\'\'\'":
            print("- \\\'\\\'\\\'：3つのアポストロフィにマッチ（エスケープが必要）")
            print("- (.+?)：1つ以上の任意の文字にマッチし、非貪欲（non-greedy）でグループ化")
            print("- \\\'\\\'\\\'：3つのアポストロフィにマッチ（エスケープが必要）")
        elif pattern == r"\'\'(.+?)\'\'":
            print("- \\\'\\\'：2つのアポストロフィにマッチ（エスケープが必要）")
            print("- (.+?)：1つ以上の任意の文字にマッチし、非貪欲（non-greedy）でグループ化")
            print("- \\\'\\\'：2つのアポストロフィにマッチ（エスケープが必要）")

# 正規表現のパターンを詳しく説明
explain_regex_patterns()'''))
